    ...
    if (this.props.stripe) {
      ...
      this.props.stripe.createToken().then(result => {
        if (result.error) {
          ...
          authAxios
            .post(handlePaymentURL, {
              stripeToken: result.token.id,
              selectedBillingAddress,
              selectedShippingAddress
            })
            .then(res => {
              this.setState({ loading: false, success: true, res });
            })
            .catch(err => {
              this.setState({ error: err, loading: false });
            });
    ...
    return (
      <Container>
        ...
        {billingAddresses.length > 0 ? (
          ...
          <Header>
            <Link to="/profile">add billing addresses</Link>
          </Header>
        )}
        {shippingAddresses.length > 0 ? (
          ...
          <Header>
            <Link to="/profile">add shipping addresses</Link>
          </Header>
        )}
        {selectedShippingAddress.length < 1 ||
        selectedBillingAddress.length < 1 ? (
          <React.Fragment>
            <Divider />
            <Message warning>
