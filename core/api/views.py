...
class FilterListView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return Response(
            {"Category": category, "Origin": origin, "Label": label}, status=HTTP_200_OK
        )
...
class OrderItemRemoveView(DestroyAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if order_qs.exists():
            ...
            if order.items.filter(item__slug=item.slug).exists():
                ...
                return Response(
                    {"message": "This item quantity was updated."}, status=HTTP_200_OK
                )
            ...
            return Response(
                {"You do not have an active order"}, status=HTTP_400_BAD_REQUEST
            )
...
class AddToCartView(APIView):
    def post(self, request, *args, **kwargs):
        ...
        if slug is None:
            return Response({"message": "Invalid request"}, status=HTTP_400_BAS_REQUEST)
        item = get_object_or_404(Item, slug=slug)
        ...
        for variation in variations:
            order_item_qs = order_item_qs.filter(item_variations__exact=variation)
